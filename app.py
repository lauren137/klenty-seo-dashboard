...
def fetch_all_klenty(start: str, end: str, limit: int = 200):
    ...
    Returns a pandas DataFrame ready for the perf table."""
    gsc = gc.gsc_all_pages(start, end, limit=limit)
    ga4 = gc.ga4_all_pages(start, end, limit=limit)
    ...
    gsc_norm = {norm(p): v for p, v in gsc.items()}
    ga4_norm = {norm(p): v for p, v in ga4.items()}
    all_paths = set(gsc_norm.keys()) | set(ga4_norm.keys())
...
with tab2:
    ...
    for _, r in df_all.iterrows():
        rows_html_all.append(
            "<tr>"
            f"<td class='url'>{r['URL']}</td>"
            f"<td class='num'>{_fmt_float1_2(r['Position'])}</td>"
            f"<td class='num'>{_fmt_int2(r['Clicks'])}</td>"
            f"<td class='num'>{_fmt_int2(r['Impressions'])}</td>"
            f"<td class='num'>{_fmt_pct_2(r['CTR %'])}</td>"
            f"<td class='num'>{_fmt_int2(r['Organic Traffic'])}</td>"
            f"<td class='num'>{_fmt_int2(r['Sessions'])}</td>"
            f"<td class='num'>{_fmt_int2(r['Users'])}</td>"
            f"<td class='num'>{_fmt_pct1_2(r['Bounce %'])}</td>"
            "</tr>"
        )

    total_row_html_all = ""
    if len(df_all) > 0:
        def pct_cell2(value, site_total):
            pct = (value / site_total * 100) if site_total else 0
            return f"<td class='num'>{value:,}<div class='pct'>{pct:.1f}% of site</div></td>"

        ta_clicks = int(df_all['Clicks'].sum())
        ta_impr = int(df_all['Impressions'].sum())
        ta_org = int(df_all['Organic Traffic'].sum())
        ta_sess = int(df_all['Sessions'].sum())
        ta_users = int(df_all['Users'].sum())

        total_row_html_all = (
            "<tr class='total-row'>"
            f"<td class='url'>TOTAL · {len(df_all)} URLs</td>"
            "<td class='num'>—</td>"
            + pct_cell2(ta_clicks, g['clicks'])
            + pct_cell2(ta_impr, g['impressions'])
            + "<td class='num'>—</td>"
            + pct_cell2(ta_org, ao['sessions'])
            + pct_cell2(ta_sess, a['sessions'])
            + pct_cell2(ta_users, a['users'])
            + "<td class='num'>—</td>"
            + "</tr>"
        )
